...
void send_variables(float *num1, float *num2, float *num3){
	int8_t *p;
	p = num1;
	while( uart_write(UART0, p[0]) != 0 );
	while( uart_write(UART0, p[1]) != 0 );
	while( uart_write(UART0, p[2]) != 0 );
	while( uart_write(UART0, p[3]) != 0 );
